...
Key innovations:
1. Polar decomposition: w = r * u(theta)
2. Freeze magnitude r, adapt only direction theta (50% parameter reduction)
3. Spherical gradient descent for natural geometry
4. Integration with TurboQuant for 8x cost reduction
"""
...
from adn.qttt.adaptation import KVCache, compute_attention_with_query
...
class PolarQTTTConfig:
    ...
    Loss Function (Sec 3.3.2 - Sec 3.3.3):
    ...
    - "margin_maximization" (alternative): Explicit margin maximization.
      Useful for fine-grained confidence calibration (Sec 3.3.3).

    Paper Defaults (Sec 3.3.4):
    ...
    # Paper default: 10 steps (Sec 3.3.4)
    num_steps: int = 10
    # Paper default: 0.01 for short sequences (Sec 3.3.4)
    learning_rate: float = 0.01
    span_length: int = 128

    # Loss function selection (Sec 3.3.2 - Sec 3.3.3)
    loss_type: str = "cross_entropy"  # "cross_entropy" or "margin_maximization"

    # Polar-specific settings
    adapt_magnitude: bool = False  # False = freeze r, adapt only theta
    adapt_direction: bool = True  # True = adapt theta
    use_spherical_sgd: bool = True  # Use Riemannian optimization on sphere

    # RaBitQ integration
    use_rabitq: bool = True  # Enable compressed execution
    rabitq_bits: int = 1  # Total bits: 1=16x, 2=8x, 3=5.3x (vs FP16)
    ...
    # Margin maximization (alternative loss, Sec 3.3.3)
...
class QueryAdaptationPolarAdapter:
    ...
    Handles the decomposition w = r * u(theta) and provides
    ...
    def __init__(self, magnitude: torch.Tensor, direction: torch.Tensor, config: PolarQTTTConfig):
        ...
        Args:
            magnitude: Scalar r
            direction: Unit vector u(theta) [d]
...
class PolarQTTT(nn.Module):
    ...
    Integrates with RaBitQ for 8x cost reduction.
    """
    ...
    def adapt_pseudo_query(
        ...
        direction: torch.Tensor,  # Unit vector u(theta) [d]
        kv_cache: KVCache,
        ...
        Args:
            ...
            direction: Direction u(theta) [d] (adapted)
    ...
    def adapt_query_projection(
        ...
        for step in range(self.config.num_steps):
            ...
            if model is not None and input_ids is not None and kv_caches is not None:
                ...
                # Default: cross_entropy (Sec 3.3.2), alternative: margin_maximization (Sec 3.3.3)
                loss = self._compute_adaptation_loss(
                    logits,
                    (
                        seq_positions
    ...
    def _compute_adaptation_loss(
        ...
        Supports two loss functions as described in Sec 3.3.2 - Sec 3.3.3:

        1. "cross_entropy" (default): Self-supervised next-token prediction.
           This is the primary training signal described in Algorithm Sec 3.3.2.
        ...
        if self.config.loss_type == "cross_entropy":
            # Cross-entropy loss (default, Sec 3.3.2)
            ...
            # Margin maximization loss (alternative, Sec 3.3.3)
            # If target has fewer positions than logits, use only the last positions
            if target_token_ids.size(1) < T:
                logits_subset = logits[:, -target_token_ids.size(1) :, :]  # [B, target_T, V]
    ...
    def compute_effective_cost(
        ...
        # RaBitQ discount - matches paper Sec 3.4: 16x for 1-bit vs FP16
        if self.config.use_rabitq:
            # 16x reduction for 1-bit (vs FP16 baseline)
...
class DepthPriorityController:
    ...
    def __init__(
        ...
        # Policy: With TurboQuant, depth is 8x cheaper
