...
class QualityScore:
    """Callable wrapper around :func:`compute_quality_score` for use in pipelines.

    Example::

        scorer = QualityScore(low_pass_ratio=0.25, window_size=512)
        rho = scorer(hidden_states)  # same as compute_quality_score(...)
    """
    ...
    def __call__(self, signal: torch.Tensor) -> torch.Tensor:
        """Compute ``rho(t)`` for ``signal`` with the configured hyperparameters."""
        return compute_quality_score(
            signal,
            low_pass_ratio=self.low_pass_ratio,
            eps=self.eps,
