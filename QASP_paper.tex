...
\begin{itemize}
    \item \textbf{Framework:} PyTorch (reference implementation released with this work)
    \item \textbf{Attention:} Multi-head attention as in the QASP model code
    \item \textbf{Quantization:} RaBitQ-style 1-bit KV codec in PyTorch (int8 signs with per-vector $\ell_2$ norms)
    \item \textbf{Manifold:} Newton--Schulz Stiefel projection in PyTorch
\end{itemize}
...
\begin{itemize}
    \item \textbf{Newton--Schulz (reference code):} 5 iterations per call to the Stiefel projector; each call renormalizes and iterates from scratch (no cached warm-start state across steps).
    \item \textbf{Value weights (reference code):} Per-token FFT along $d$ as in \eqref{eq:quality-score}. Sliding-window batching and cross-token caching (Sec.~\ref{sec:sliding-window}, $W{=}512$) are not implemented in the released package.
    \item \textbf{Proxy-model training (Table~\ref{tab:hyperparams}):} BF16 mixed precision, gradient checkpointing on selected layers, and optional \texttt{torch.compile} were used in the pipeline that produced the 1.5B results; these choices are training-stack concerns and are not imposed by the released PyTorch modules (which default to FP32 unless a script sets mixed precision).
\end{itemize}
