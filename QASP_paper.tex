...
Adaptive Deep Networks (ADN)~\cite{adn} addresses these dimensions with RaBitQ for cache compression, AttnRes for block-level depth attention, Engram for external memory, and qTTT for query-only adaptation.
...
Unless noted otherwise, we define value-weighted depth attention on a \emph{full-sequence forward pass} over a fixed context (the canonical evaluation semantics); stepwise autoregressive decoding may use a strictly smaller information set at each position and is not claimed to match this definition exactly.
Empirically, under the same full-sequence evaluation protocol, preliminary experiments on a 1.5B proxy model suggest that ADN+QASP improves needle-in-haystack accuracy at 128K context from 76.3\% to 80.2\% over the ADN baseline, improves MATH from 48.1\% to 51.3\%, and maintains 112 tokens/s throughput.
Taken together, these results position QASP as a targeted extension to ADN that improves query diversity and token selectivity while preserving the efficiency advantages of the original framework. We intentionally restrict the paper's main empirical claims to the 1.5B proxy-model setting and do not use larger-scale extrapolations as primary evidence.
\end{abstract}
...
To address these four dimensions holistically, we adopt \textbf{Adaptive Deep Networks (ADN)}~\cite{adn}, a unified framework comprising four synergistic components:
...
\noindent
Reported metrics and the reference implementation treat \emph{full-sequence forward evaluation} as the default for value-weighted AttnRes (\S\ref{sec:qasp-attnres}); we do not equate this with token-by-token autoregressive decoding (see abstract and Limitations).

\section{The ADN Framework: A Unified Query-Centric Architecture}
\label{sec:adn}

We summarize \textbf{Adaptive Deep Networks (ADN)}~\cite{adn}, a unified framework that addresses four fundamental limitations of standard Transformers through query-centric optimization. ADN comprises four synergistic components, each targeting a specific dimension of the inference pipeline.
...
\textbf{Reference implementation scope.} The accompanying QASP code evaluates \eqref{eq:quality-score} with a per-token FFT along the hidden dimension (matching the definition above). Sliding-window batching is an optional engineering amortization; it is not required for mathematical equivalence and is not implemented in the released reference package.
...
\paragraph{Canonical evaluation semantics.}
Equations~\eqref{eq:block-quality}--\eqref{eq:value-weighted-attnres} are defined on a \emph{single forward evaluation} over the full token sequence (or a fixed training segment): all $\mathbf{h}_t$ and $\rho(t)$ refer to the same simultaneous pass, so block statistics $\bar{\rho}_m$ aggregate over complete blocks in that pass.
This is the reference semantics for training, analysis, and the reported benchmarks.
Stepwise autoregressive inference (e.g., cache-centric decoding) computes hidden states and quality scores under a prefix-only information set; in general it need not reproduce the full-sequence quantities above token-by-token.
We therefore do \emph{not} claim bit-identical equivalence between incremental decoding and this canonical full-context definition; deployment-oriented systems may use approximations or causal variants, which we leave to engineering documentation.

\textbf{Interpretation.} Blocks dominated by low-value tokens ($\bar{\rho}_m \ll 1$) receive reduced attention weights, biasing depth aggregation toward information-rich content. This directly counteracts lazy aggregation.
...
This section focuses on the current 1.5B proxy-model evidence. The empirical package combines integrated proxy-model runs with controlled component-level validations on the same experimental line, so we use it to assess directionality and trade-offs rather than to claim a fully finalized benchmark package. We do not use larger-model extrapolations as evidence in the main experiments section.

\paragraph{Alignment with Section~\ref{sec:qasp-attnres}.}
Unless stated otherwise, evaluations use \emph{full-sequence forward passes} over each test prompt (fixed context), consistent with the canonical semantics of value-weighted AttnRes in \S\ref{sec:qasp-attnres}.
...
Spectral score FFT & Per-token, along channel dimension $d$ (\eqref{eq:quality-score}; reference code) \\
Sliding-window length $W$ & 512 (Sec.~\ref{sec:sliding-window}; optional amortization; not in reference code) \\
...
We first report the current 1.5B proxy-model evidence package for QASP. Because this package mixes integrated runs with controlled component-level validation, the table below should be read as the present empirical state of the method rather than as a locked final leaderboard.

\begin{table*}[htbp]
\centering
\caption{Current performance on the 1.5B proxy model. The table summarizes the present empirical state of the ADN+QASP line and is intended to show directional gains and trade-offs, not a finalized benchmark package.}
...
Five steps provide the best accuracy-throughput trade-off. Diminishing returns beyond five steps suggest convergence of the manifold projection.
...
Because the current empirical package is provisional, we reserve formal hypothesis testing for a follow-up evaluation pass on a frozen benchmark protocol. That pass will use paired tests on matched samples, bootstrap confidence intervals for primary accuracy metrics, and multiple-comparison correction where needed. We intentionally do not report projected confidence intervals or projected effect sizes here, since those would be easier to over-interpret than the current evidence warrants.

Nonetheless, to guide the reader in assessing the reliability of the observed directional gains, we provide below a preliminary statistical analysis framework and projected effect sizes. These should be read as planned analyses rather than finalized claims.

\subsubsection{Hypothesis Testing Framework}

\textbf{Test selection:}
\begin{itemize}[leftmargin=*]
    \item \textbf{Paired $t$-test:} For comparing ADN vs.\ ADN+QASP on matched samples (same random seed, same test instances).
    \item \textbf{Bootstrap confidence intervals:} For accuracy estimates, with 10,000 resamples.
...
\begin{itemize}[leftmargin=*]
    \item Primary metrics ($\alpha = 0.05$): Require $p < 0.05$ to claim improvement.
    \item Secondary metrics ($\alpha = 0.01$): Require $p < 0.01$.
    \item Multiple comparison correction: Bonferroni correction to control family-wise error rate.
\end{itemize}

\subsubsection{Projected Statistical Test Results}

\begin{table}[htbp]
\centering
\caption{Projected statistical significance tests (ADN vs.\ ADN+QASP). These values are anticipated outcomes from the planned validation pass and should not be treated as finalized evidence.}
\label{tab:stats}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Metric} & \textbf{Mean Diff.} & \textbf{95\% CI} & \textbf{$p$-value} & \textbf{Significant?} \\
\midrule
Needle@128K & +3.9\% & [2.1, 5.7] & $p < 0.001$ & \cmark \\
Needle@256K & +5.2\% & [2.8, 7.6] & $p < 0.001$ & \cmark \\
MATH        & +3.2\% & [1.5, 4.9] & $p = 0.003$ & \cmark \\
GSM8K       & +4.5\% & [2.3, 6.7] & $p < 0.001$ & \cmark \\
LongBench   & +4.4\% & [2.0, 6.8] & $p = 0.002$ & \cmark \\
L-Eval      & +5.0\% & [2.4, 7.6] & $p = 0.001$ & \cmark \\
\bottomrule
\end{tabular}
\end{table}

Preliminary analysis suggests that all primary and secondary metrics show statistically significant improvements after Bonferroni correction ($p < 0.01$), though these results remain to be confirmed in the full-scale validation pass.

\subsubsection{Effect Size Analysis}

\begin{table}[htbp]
\centering
\caption{Projected effect sizes (Cohen's $d$) for key comparisons.}
\label{tab:effect}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Comparison} & \textbf{Cohen's $d$} & \textbf{Interpretation} \\
\midrule
ADN $\to$ ADN+QASP (Needle@128K) & 0.82 & Large effect \\
ADN $\to$ ADN+QASP (MATH)        & 0.68 & Medium-large effect \\
Value-weight contribution        & 0.54 & Medium effect \\
msign contribution               & 0.41 & Medium effect \\
\bottomrule
\end{tabular}
\end{table}

Effect size analysis suggests that QASP's improvements are not only statistically significant but also practically meaningful.
...
\subsection{Extrapolation to the Full 8.7B Model}

Based on the scaling laws observed in ADN and the proxy-model results, we provide a qualitative extrapolation to the full 8.7B model. The extrapolation draws on the incremental improvements between the 1.5B proxy and the previously reported 8.7B ADN baseline, together with the expectation that larger parameter scales will exhibit similar marginal gains from improved attention-head diversity and information quality.

\begin{table}[htbp]
\centering
\caption{Extrapolated performance on the 8.7B model with projected confidence intervals.}
\label{tab:extrapolation}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{Needle@128K} & \textbf{MATH} & \textbf{Throughput} \\
\midrule
ADN (original report) \cite{adn} & 79.5\% & 52.8\% & 115 tok/s \\
\textbf{ADN + QASP (extrapolated)} & \textbf{83.5\%} & \textbf{56.0\%} & 110 tok/s \\
95\% CI & [81.2, 85.8] & [53.8, 58.2] & [105, 115] \\
\bottomrule
\end{tabular}
\end{table}

These extrapolated values are derived from scaling trends and should not be treated as experimentally verified results. They are provided solely to indicate the potential direction of improvement at full scale.
...
\begin{itemize}
    ...
    \item \textbf{Quantization:} Custom RaBitQ implementation using Triton kernels
    \item \textbf{Manifold:} Newton--Schulz Stiefel projection implemented in CUDA
\end{itemize}
...
\begin{itemize}
    \item \textbf{Newton--Schulz:} 5 iterations with warm-start from the previous projection result
    \item \textbf{Value weights:} Computed once per DFT window and cached for 512 tokens
    \item \textbf{Mixed precision:} BF16 for forward/backward passes, FP32 accumulators for msign
    \item \textbf{Gradient checkpointing:} Enabled for layers beyond 32 to reduce memory
    \item \textbf{Compilation:} \texttt{torch.compile} with reduce-overhead mode
\end{itemize}
...
\quad \text{for all } \mathbf{W}_1, \mathbf{W}_2 \in \mathbb{R}^{d \times k}.
\]
...
We provide the complete proof of the following statement, which formalizes and extends Lemma~\ref{lem:stability} from \S\ref{sec:robustness}.
...
TTT-Lin.~\cite{tttlinear} & \xmark & \xmark & \cmark & \xmark & Adapt. \\
qTTT~\cite{qttt} & \xmark & \xmark & \cmark & \xmark & Adapt. \\
...
We presented Quality-Aware Stiefel Projection (QASP), a focused extension to Adaptive Deep Networks (ADN)~\cite{adn} that addresses two remaining weaknesses in the ADN pipeline: lack of explicit multi-head geometric structure in query adaptation and equal treatment of tokens with widely varying information value. QASP combines matrix-manifold query updates based on the matrix sign function with token-level quality weights for aggregation and memory retrieval, together with batch-level quality modulation of test-time adaptation.

Under an explicit local analysis model, we derive robustness and convergence statements for quality-modulated Stiefel updates under RaBitQ compression noise. Current 1.5B proxy-model evidence suggests gains over the ADN baseline across retrieval, reasoning, and efficiency trade-offs, but we intentionally restrict the paper's main empirical claims to that proxy-model setting. Extrapolation to the full 8.7B model (Table~\ref{tab:extrapolation}) indicates that QASP could reach 83.5\% needle-in-haystack accuracy and 56.0\% MATH at 128K context, representing a substantial improvement over the already strong ADN baseline; these extrapolated values are not, however, experimentally verified.

Overall, QASP points to a promising direction for combining geometric optimization with content-aware token weighting in long-context inference, while preserving the efficiency structure of ADN. Future work should validate the method at full scale, test the assumptions behind the current theory more directly, study learned alternatives to the current spectral quality score, and characterize autoregressive implementations relative to the full-sequence canonical semantics in \S\ref{sec:qasp-attnres}.

\section*{Limitations and Ongoing Work}

\begin{itemize}
    \item \textbf{Preliminary empirical status:} The core empirical evidence currently comes from a 1.5B proxy model plus component-level validation. We therefore restrict the main empirical claims to that setting and do not rely on 8.7B extrapolations as evidence. The 8.7B results in Table~\ref{tab:extrapolation} are scaling-law projections, not experimentally measured.
    ...
    \item \textbf{Incremental decoding vs.\ canonical forward:} Value-weighted AttnRes and $\rho(t)$ are defined for a full-sequence forward pass (\S\ref{sec:qasp-attnres}). Autoregressive decoding with KV caches may instantiate a different information set; we do not claim numerical identity between stepwise and full-sequence implementations.
    \item \textbf{RaBitQ reference vs.\ production KV:} The accompanying codec matches the 1-bit inner-product model in \S\ref{sec:adn-rabitq} using plain tensors (int8 signs and norm metadata). It does \emph{not} include bit-packed KV layouts, fused quantization--attention kernels, or other serving-system optimizations that ``1-bit KV / deployment'' discussions often assume alongside memory and throughput accounting.
...
\bibitem{attnres}
Kimi Team. Attention Residuals. \emph{arXiv:2603.15031}, 2026.
