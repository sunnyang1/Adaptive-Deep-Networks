...
|------|------|-----------|------|------------|
| **Small (2.2B)** | 单卡 | 8 | `python scripts/training/train_model.py --model-size small` | ~20GB |
| **Small (2.2B)** | 4卡并行 | 32 | `torchrun --nproc_per_node=4 ...` | ~20GB |
| **Medium (8.7B)** | 单卡 | 2 | `python scripts/training/train_model.py --model-size medium` | ~90GB |
